...
if (!$settings['module2_enabled'] && !is_admin()) {
  header('Location: dashboard.php?error=Módulo%202%20bloqueado');
  exit;
}
$subs = subjects_by_module(2);
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="min-h-screen flex flex-col">
  ...
  <main class="flex-1 container mx-auto px-4 py-8">
    <div class="text-center mb-10">
      <h1 class="text-3xl font-bold mb-2">Materias del Módulo 2</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 max-w-6xl mx-auto">
      <?php foreach ($subs as $s): ?>
        <a href="subject.php?id=<?= urlencode($s['slug']) ?>&modulo=2" class="subject-card">
