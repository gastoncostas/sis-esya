...
// Buscar cursante
$sql = "SELECT * FROM cursantes WHERE id = ?";
...
if ($resultado->num_rows === 0) {
    header("Location: index.php?error=Cursante no encontrado");
    exit;
}

$cursante = $resultado->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($dni) || empty($nombre) || empty($apellido)) {
        ...
        $checkDni = $conn->prepare("SELECT id FROM cursantes WHERE dni = ? AND id != ?");
        $checkDni->bind_param("si", $dni, $id);
        $checkDni->execute();
        $checkDni->store_result();

        if ($checkDni->num_rows > 0) {
            $error = 'El DNI ya está registrado en el sistema para otro cursante';
        } else {
            // Actualizar cursante
            $stmt = $conn->prepare("UPDATE cursantes SET 
                ...
                novedades=?, estado=?, updated_at=NOW()
                WHERE id=?");

            $stmt->bind_param(
                "sssssiisiisssssiisssissssi",
                $dni,
                $apellido,
                $nombre,
                $fecha_nacimiento,
            ...
            if ($stmt->execute()) {
                ...
                // Actualizar datos del cursante para mostrar en el formulario
                $cursante = array_merge($cursante, $_POST);
                $cursante['vive_padre'] = $vive_padre;
                $cursante['vive_madre'] = $vive_madre;
            } else {
                $error = 'Error al actualizar cursante: ' . $stmt->error;
            }
        ...
        <form method="post" action="">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="dni" name="dni" value="<?php echo htmlspecialchars($cursante['dni']); ?>" required maxlength="8" pattern="[0-9]{7,8}">
                    <small>Solo números, 7-8 dígitos</small>
                </div>

                <div class="form-group">
                    <label for="nombre" class="required">Nombre</label>
                    <input type="text" id="nombre" name="nombre" value="<?php echo htmlspecialchars($cursante['nombre']); ?>" required maxlength="100">
                </div>

                <div class="form-group">
                    <label for="apellido" class="required">Apellido</label>
                    <input type="text" id="apellido" name="apellido" value="<?php echo htmlspecialchars($cursante['apellido']); ?>" required maxlength="100">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="fecha_nacimiento" name="fecha_nacimiento" value="<?php echo htmlspecialchars($cursante['fecha_nacimiento']); ?>">
                </div>

                <div class="form-group">
                    <label for="fecha_ingreso">Fecha de Ingreso</label>
                    <input type="date" id="fecha_ingreso" name="fecha_ingreso" value="<?php echo htmlspecialchars($cursante['fecha_ingreso']); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="comision_id" name="comision_id">
                        ...
                        <?php foreach ($comisiones as $comision): ?>
                            <option value="<?php echo $comision['id']; ?>" <?php echo ($cursante['comision_id'] == $comision['id']) ? 'selected' : ''; ?>>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select id="nivel_educativo" name="nivel_educativo">
                        <option value="SECUNDARIO" <?php echo ($cursante['nivel_educativo'] == 'SECUNDARIO') ? 'selected' : ''; ?>>Secundario</option>
                        <option value="TERCIARIO" <?php echo ($cursante['nivel_educativo'] == 'TERCIARIO') ? 'selected' : ''; ?>>Terciario</option>
                        <option value="UNIVERSITARIO" <?php echo ($cursante['nivel_educativo'] == 'UNIVERSITARIO') ? 'selected' : ''; ?>>Universitario</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="hijos">Número de Hijos</label>
                    <input type="number" id="hijos" name="hijos" value="<?php echo htmlspecialchars($cursante['hijos']); ?>" min="0" max="20">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="nombre_padre" name="nombre_padre" value="<?php echo htmlspecialchars($cursante['nombre_padre']); ?>" maxlength="200">
                </div>

                <div class="form-group">
                    <label for="nombre_madre">Nombre de la Madre</label>
                    <input type="text" id="nombre_madre" name="nombre_madre" value="<?php echo htmlspecialchars($cursante['nombre_madre']); ?>" maxlength="200">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="direccion_real" name="direccion_real" value="<?php echo htmlspecialchars($cursante['direccion_real']); ?>" maxlength="255">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="departamento_id" name="departamento_id">
                        ...
                        <?php foreach ($departamentos as $depto): ?>
                            <option value="<?php echo $depto['id']; ?>" <?php echo ($cursante['departamento_id'] == $depto['id']) ? 'selected' : ''; ?>>
                    ...
                    <select id="localidad_id" name="localidad_id">
                        ...
                        <?php foreach ($localidades as $localidad): ?>
                            <option value="<?php echo $localidad['id']; ?>"
                                ...
                                <?php echo ($cursante['localidad_id'] == $localidad['id']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="cod_postal" name="cod_postal" value="<?php echo htmlspecialchars($cursante['cod_postal']); ?>" maxlength="10">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="tel" id="telefono" name="telefono" value="<?php echo htmlspecialchars($cursante['telefono']); ?>" maxlength="20">
                </div>

                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($cursante['email']); ?>" maxlength="150">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="nombre_fam_directo" name="nombre_fam_directo" value="<?php echo htmlspecialchars($cursante['nombre_fam_directo']); ?>" maxlength="200">
                </div>

                <div class="form-group">
                    <label for="tel_fam_directo">Teléfono Familiar</label>
                    <input type="tel" id="tel_fam_directo" name="tel_fam_directo" value="<?php echo htmlspecialchars($cursante['tel_fam_directo']); ?>" maxlength="20">
                </div>

                <div class="form-group">
                    <label for="parentezco">Parentezco</label>
                    <input type="text" id="parentezco" name="parentezco" value="<?php echo htmlspecialchars($cursante['parentezco']); ?>" maxlength="100">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="novedades" name="novedades" rows="4"><?php echo htmlspecialchars($cursante['novedades']); ?></textarea>
            </div>
